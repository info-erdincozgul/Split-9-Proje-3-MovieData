...
const App = (props) => {
  ...
  const [darkMode, setDarkMode] = useLocalStorage('s11d3', []);
    ...
    sendDeleteRequest({
      ...
      callbackSuccess: () => {
        ...
        setFavMovies((prevFavMovies) =>
          prevFavMovies.filter((favMovie) => favMovie.id !== id)
        );
      },
    });
  };
  ...
  const addToFavorites = (movie) => {
    const oneTimeCheck = favMovies.some((favmovie) => favmovie.id === movie.id);
  ...
  return (
    <div
      id="main-container"
      className={darkMode ? 'dark bg-slate-900 h-screen' : undefined}
    >
      <nav className=" bg-zinc-800 text-white px-6 py-3 dark:bg-gray-800 ">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          <span className="ms-3  font-medium text-gray-900 dark:text-gray-300">
            {darkMode ? 'Dark Mode On' : 'Dark Mode Of'}
          </span>
        </label>
      </nav>
